...
void CristoFinity::MenuAdministrador(bool &acaba){
	...
	do{
		...
		cout << amarillofuerte << "[13] " << cianfuerte << "Imprimir busquedas de Usuarios normales" << rest << endl;
		...
		cout << amarillofuerte << "[18] " << cianfuerte << "Imprimir base de datos" << rest << endl;

		cout << endl;
		cout << rojo << "[19] " << cianfuerte << "Salir/Cambiar Usuario" << rest << endl;
...
void CristoFinity::MenuUserNormal(bool &acaba){
	...
	do{
		...
		cout << endl;
		cout << rojo << "[14] " << cianfuerte << "Salir/Cambiar Usuario" << rest << endl;
...
void CristoFinity::calcularTop3SerieseInsertarEnVectorBusquedas(){
	...
	if(TotalMediaCatalogo > 2){
		for(int i = 0; i < 3; i++){
			cont = 0;
			for(int j = 0; j < TotalMediaCatalogo; j++){
				if(UsuarioActivo->compruebaRank(Catalogo[j]) == true){
					if(Serie *se1 = dynamic_cast<Serie *>(Catalogo[j])){
						...
						//cout << rojo << "se metee " << se1->getTitulo() << rest << endl;
						for(int k = 0; k < TotalMediaCatalogo; k++){
							if(UsuarioActivo->compruebaRank(Catalogo[k]) == true){
								if(Serie *se2 = dynamic_cast<Serie *>(Catalogo[k])){
									//cout << rojo << "se metee 2" << se2->getTitulo() << rest << endl;
									if(se1->getNotaGeneral() <= se2->getNotaGeneral()){
										if(Catalogo[posicion_mayor]->getNotaGeneral() < se2->getNotaGeneral() ){
											posicion_mayor = k;
											//cout << rojo << "san metio " << Catalogo[posicion_mayor]->getTitulo() << rest << endl;
										}
									}
								}
							}
			...
			if(Serie *se3 = dynamic_cast<Serie *>(Catalogo[posicion_mayor])){
			UsuarioActivo->insertarMediaEnListaDeBusqueda(se3);
		}
		}
	}
	else{
		cout << rojo << "Lo siento, no hay suficientes series para hacer un top 3 " << rest << endl;
	}
}
...
void CristoFinity::calcularTop3PeliculaseInsertarEnVectorBusquedas(){
	...
	if(TotalMediaCatalogo > 2){
		for(int i = 0; i < 3; i++){
			...
			for(int j = 0; j < TotalMediaCatalogo; j++){
				if(UsuarioActivo->compruebaRank(Catalogo[j]) == true){
					if(Pelicula *pel1 = dynamic_cast<Pelicula *>(Catalogo[j])){
						...
						//cout << rojo << "se metee " << pel1->getTitulo() << rest << endl;
						for(int k = 0; k < TotalMediaCatalogo; k++){
							if(UsuarioActivo->compruebaRank(Catalogo[k]) == true){
								if(Pelicula *pel2 = dynamic_cast<Pelicula *>(Catalogo[k])){
									//cout << rojo << "se metee 2" << pel2->getTitulo() << rest << endl;
									if(pel1->getNotaGeneral() <= pel2->getNotaGeneral()){
										if(Catalogo[posicion_mayor]->getNotaGeneral() < pel2->getNotaGeneral() ){
											posicion_mayor = k;
											//cout << rojo << "san metio " << Catalogo[posicion_mayor]->getTitulo() << rest << endl;
										}
									}
			...
			if(Pelicula *pel3 = dynamic_cast<Pelicula *>(Catalogo[posicion_mayor])){
			UsuarioActivo->insertarMediaEnListaDeBusqueda(pel3);
		}
		}
	}
	else{
		cout << rojo << "Lo siento, no hay suficientes peliculas para hacer un top 3 " << rest << endl;
	}
}
...
void CristoFinity::calcularTop3VideojuegoseInsertarEnVectorBusquedas(){
	...
	if(TotalMediaCatalogo > 2){
		for(int i = 0; i < 3; i++){
			...
			for(int j = 0; j < TotalMediaCatalogo; j++){
				if(UsuarioActivo->compruebaRank(Catalogo[j]) == true){
					if(Videojuego *vid1 = dynamic_cast<Videojuego *>(Catalogo[j])){
						...
						//cout << rojo << "se metee " << vid1->getTitulo() << rest << endl;
						for(int k = 0; k < TotalMediaCatalogo; k++){
							if(UsuarioActivo->compruebaRank(Catalogo[k]) == true){
								if(Videojuego *vid2 = dynamic_cast<Videojuego *>(Catalogo[k])){
									//cout << rojo << "se metee 2" << vid2->getTitulo() << rest << endl;
									if(vid1->getNotaGeneral() <= vid2->getNotaGeneral()){
										if(Catalogo[posicion_mayor]->getNotaGeneral() < vid2->getNotaGeneral() ){
											posicion_mayor = k;
											//cout << rojo << "san metio " << Catalogo[posicion_mayor]->getTitulo() << rest << endl;
										}
									}
			...
			if(Videojuego *vid3 = dynamic_cast<Videojuego *>(Catalogo[posicion_mayor])){
			UsuarioActivo->insertarMediaEnListaDeBusqueda(vid3);
		}
		}
	}
	else{
		cout << rojo << "Lo siento, no hay suficientes videojuegos para hacer un top 3 " << rest << endl;
	}
}
...
void CristoFinity::calcularTop3CancioneseInsertarEnVectorBusquedas(){
	...
	if(TotalMediaCatalogo > 2){
		for(int i = 0; i < 3; i++){
			...
			for(int j = 0; j < TotalMediaCatalogo; j++){
				if(UsuarioActivo->compruebaRank(Catalogo[j]) == true){
					if(Cancion *can1 = dynamic_cast<Cancion *>(Catalogo[j])){
						...
						//cout << rojo << "se metee " << can1->getTitulo() << rest << endl;
						for(int k = 0; k < TotalMediaCatalogo; k++){
							if(UsuarioActivo->compruebaRank(Catalogo[k]) == true){
								if(Cancion *can2 = dynamic_cast<Cancion *>(Catalogo[k])){
									//cout << rojo << "se metee 2" << can2->getTitulo() << rest << endl;
									if(can1->getNotaGeneral() <= can2->getNotaGeneral()){
										if(Catalogo[posicion_mayor]->getNotaGeneral() < can2->getNotaGeneral() ){
											posicion_mayor = k;
											//cout << rojo << "san metio " << Catalogo[posicion_mayor]->getTitulo() << rest << endl;
										}
									}
			...
			if(Cancion *can3 = dynamic_cast<Cancion *>(Catalogo[posicion_mayor])){
			UsuarioActivo->insertarMediaEnListaDeBusqueda(can3);
		}
		}
	}
	else{
		cout << rojo << "Lo siento, no hay suficientes canciones para hacer un top 3 " << rest << endl;
	}
}
...
void CristoFinity::rankingUsuarios(){
	...
	cout << magentafuerte << "Elige el ranking en base al tipo de usuario que quieres ver " << rest << endl;

	cout << amarillofuerte << "[1] " << cianfuerte << "Usuario Freak-Zizou " << rest << endl;
	cout << amarillofuerte << "[2] " << cianfuerte << "Usuario Salvame Deluxe" << rest << endl;
	cout << amarillofuerte << "[3] " << cianfuerte << "Usuario Messi " << rest << endl;

	cin >> opcion;

	if(opcion == 1){
		calcularFreakZizouEInsertar();
	}else if(opcion == 2){
		calcularSalvameDeluxeEInsertar();
	}else if(opcion == 3){
		calcularMessiEInsertar();
	}

	printVectorAdministrador();
}

void CristoFinity::calcularFreakZizouEInsertar(){

	if(Administrador *ad = dynamic_cast<Administrador *> (UsuarioActivo)){
		int dimensionvectoresaux = 50;

		Usuario **aux1 = new Usuario*[dimensionvectoresaux];
		Usuario **aux2 = new Usuario*[dimensionvectoresaux];

		int j = 0;
		for(int i = 0; i < TotalUsuarios && i < dimensionvectoresaux; i++){
			aux1[i] = VectorUsuarios[i];
			aux2[i] = VectorUsuarios[i];
		}

		//ORDENAR VECTOR POR MAYOR NOTA MEDIA DE FAVORITOS AUXILIAR 1

		Usuario *auxgeneral = new Usuario;
		...
		for(int i = 0; i < TotalUsuarios && i < dimensionvectoresaux; i++){
			if(Normal *nor1 = dynamic_cast<Normal *> (aux1[i])){
				posmin1 = i;	
				for (int j = i+1; j < TotalUsuarios && j < dimensionvectoresaux; j++){
					if(Normal *nor2 = dynamic_cast<Normal *> (aux2[j])){
						if (mediaActual < nor2->getMediaFavoritos()){
							posmin1 = j;
							mediaActual = nor2->getMediaFavoritos();
						}
					}
				}
			auxgeneral = aux1[i];
		...
		//FIN ORDENACION POR MAYOR NOTA MEDIA DE FAVORITOS VECTOR AUXILIAR 1

		//ORDENAR VECTOR POR MAYOR NUMERO DE ASOCIADOS AUXILIAR 2

		int pos_min2 = 0;

		int asociadosActuales = 0;
		...
		for(int i = 0; i < TotalUsuarios && i < dimensionvectoresaux; i++){
			if(Normal *nor1 = dynamic_cast<Normal *> (aux1[i])){
				...
				for (int j = i+1; j < TotalUsuarios && j < dimensionvectoresaux; j++){
					if(Normal *nor2 = dynamic_cast<Normal *> (aux2[j])){
						if (mediaActual < nor2->getTotalAsociados()){
							posmin1 = j;
							asociadosActuales = nor2->getTotalAsociados();
						}
					}
				}
			auxgeneral = aux1[i];
		...
		//FIN ORDENACION VECTOR POR MAYOR NUMERO DE ASOCIADOS AUXILIAR 2
		...
		for(int i = 0; i < TotalUsuarios && i < dimensionvectoresaux; i++){
			if(Normal *nor1 = dynamic_cast<Normal *> (aux1[i])){
				for(int j = 0; j < TotalUsuarios && j < dimensionvectoresaux; j++){
					if(Normal *nor2 = dynamic_cast<Normal *> (aux1[j])){
						if(nor1->getLogin() == nor2->getLogin()){
							suma = (i+1) + (j+1);
							//cout << rojo << "suma" << suma << rest << endl;
							if(suma < sumaactual){
								sumaactual = suma;
								//cout << rojo << "sumaactual" << sumaactual << rest << endl;
								pos_usuario = buscarLoginUsuario(nor1->getLogin());
							}
						}
					}
				}
		...
		ad->insertarUsuarioEnVectorBusquedasUsuario(VectorUsuarios[pos_usuario]);
	}
}

void CristoFinity::calcularSalvameDeluxeEInsertar(){

	if(Administrador *ad = dynamic_cast<Administrador *>(UsuarioActivo)){
		int pos_usuario = 0;
		int comentariosActuales = 0;

		for(int i = 0; i < TotalUsuarios; i++){
			if(Normal *nor1 = dynamic_cast<Normal *> (VectorUsuarios[i])){
				for(int j = 0; j < TotalUsuarios; j++){
					if(Normal *nor2 = dynamic_cast<Normal *> (VectorUsuarios[j])){
						if(comentariosActuales < contarComentariosPorLogin(nor2->getLogin())){
							pos_usuario = j;
							comentariosActuales = contarComentariosPorLogin(nor2->getLogin());
						}	
					}
				}
			}
...
void CristoFinity::calcularMessiEInsertar(){
	if(Administrador *ad = dynamic_cast<Administrador *> (UsuarioActivo)){
		int dimensionvectoresaux = 50;
...
void CristoFinity::insertarUsuario(Usuario *u){
	...
	stringtotal = "INSERT INTO user VALUES('" + login + "', '" + nombre + "', '" + apellidos + "', '" + fotoperfil + "');"; 

	VectorUsuarios[TotalUsuarios] = u;
	resizeUsuariosMas();

	insertarEnBaseDeDatos(stringtotal);
}
...
void CristoFinity::eliminarUsuario(int posicion){
	...
	//cout << rojo << "debug 1 " << rest << endl;

	login = VectorUsuarios[posicion]->getLogin();

	//cout << rojo << "debug 2 " << rest << endl;

	sentencia = "DELETE FROM `user` WHERE `login` = '" + login + "';";
	borrarTuplaDeUsuario(sentencia);

	//	cout << rojo << "debug 3 " << rest << endl;
...
void CristoFinity::conexionBaseDatos(){

	//cout << rojo << "debug " << rest << endl;
    ...
    connection->setSchema("cristorey");

    stmt = connection->createStatement();

    stmt->execute("DROP TABLE IF EXISTS user ;");
  	//cout << "\t... Elimnamos las tabla usuario ... " << endl;

    //Creamos la tabla
  	//cout << "\t... Creamos las tabla user ... " << endl;
...
void CristoFinity::insertarEnBaseDeDatos(string sentencia){
    ...
    stmt = connection->createStatement();
 	...
 	//cout << "\t... Insertamos datos en la tabla user ... " << endl;
	stmt->execute(sentencia);
}

void CristoFinity::imprimirBaseDeDatos(){
	...
	resultset = stmt->executeQuery("SELECT login, nombre, apellidos, fotoperfil FROM user;");
	//cout << "\t... Consultamos y accedemos a los miembros por nombre de columna ..." << endl;
	while (resultset->next()) {
	  	/* Access column data by alias or column name */
	  	cout << endl;
	    cout << azulfuerte << "Login: " << cianfuerte << resultset->getString("login") << endl;
		cout << azulfuerte << "Nombre: " << cianfuerte << resultset->getString("nombre") << endl;
		cout << azulfuerte << "Apellidos: " << cianfuerte << resultset->getString("apellidos") << endl;
		cout << azulfuerte << "Foto De Perfil " << cianfuerte << resultset->getString("fotoperfil") << endl;
		cout << endl;
	}
}

void CristoFinity::borrarTuplaDeUsuario(string sentencia){
    sql::Driver *driver;
    sql::Connection *connection;
    sql::Statement *stmt;
    sql::ResultSet *resultset;
...
void CristoFinity::borrarBaseDatos(){
...
void CristoFinity::calcularMediaUsuarioNormal(){
	for(int i = 0; i < TotalUsuarios; i++){
		if(Normal *nor = dynamic_cast<Normal *>(VectorUsuarios[i])){
			nor->calcularMediaNotaMediaAsociados();
		}
	}
}

int CristoFinity::contarComentariosPorLogin(string login){

	int suma = 0;

	for(int i = 0; i < TotalMediaCatalogo; i++){
		suma = suma + Catalogo[i]->contarComentariosPorLogin(login);
	}
